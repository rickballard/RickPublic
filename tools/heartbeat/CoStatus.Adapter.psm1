# CoStatus.Adapter.psm1 — safe, idempotent Use-CoStatus implementation
...
function Use-CoStatus {
  ...
  function Get-GitBranchSafe {
    try {
      ...
      if ([string]::IsNullOrWhiteSpace($b)) { return '(no-git)' }
      return $b.Trim()
    } catch { '(no-git)' }
  }
  ...
  function Get-RepoNameSafe {
    try {
      $p = Get-Location
      if ($p) { return (Split-Path -Leaf $p.Path) } else { return '(unknown)' }
    } catch { '(unknown)' }
  }
